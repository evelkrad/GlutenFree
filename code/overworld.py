...
class Overworld:
    ...
    def draw_ui_text(self):
        font = pygame.font.Font(join('..', 'graphics', 'ui', 'PressStart2P-Regular.ttf'), 16)
        def render_centered(text, y):
            surface = font.render(text, True, (255, 255, 255))
            rect = surface.get_rect(center=(self.display_surface.get_width() // 2, y))
            for dx, dy in [(-2,0),(2,0),(0,-2),(0,2),(-2,-2),(2,-2),(-2,2),(2,2)]:
                shadow = font.render(text, True, (0, 0, 0))
                shadow_rect = shadow.get_rect(center=(rect.centerx + dx, rect.centery + dy))
                self.display_surface.blit(shadow, shadow_rect)
            self.display_surface.blit(surface, rect)

        render_centered("Pulsa ENTER para entrar en un nivel", 30)
        render_centered("Utiliza las flechas de dirección para moverte entre niveles", 60)
