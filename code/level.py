...
from transition import DeathTransition
from title import CreditScreen
...
class Level:
    ...
    def setup(self, tmx_map, level_frames, audio_files):
        ...
        for obj in tmx_map.get_layer_by_name('BG details'):
            if obj.name == 'static':
                ...
                if obj.name == 'candle':
                    AnimatedSprite((obj.x,obj.y)+ vector(-20,-20),level_frames['candle_light'], self.all_sprites, z = Z_LAYERS['bg tiles'], animation_speed = ANIMATION_SPEED)
        ...
        for obj in tmx_map.get_layer_by_name('Enemies'):
            if obj.name == 'bread':
                Tooth((obj.x, obj.y), level_frames['bread'], (self.all_sprites, self.damage_sprites, self.tooth_sprites), self.collision_sprites)
    ...
    def check_constraint(self):
        ...
        #Bottom
        if self.player.hitbox_rect.bottom > self.level_bottom and not self.death_transition:
            self.death_sound.play()
            self.death_transition = DeathTransition(self.display_surface)


        #Success
        if self.player.hitbox_rect.colliderect(self.level_finish_rect):
            if self.level_unlock == 6:
                CreditScreen(self.display_surface).run()
            else:
    ...
    def run(self, dt):
        ...
        if self.death_transition:
            done = self.death_transition.run(dt)
            if done:
